...
def run_component(df, options={}):
    return pc.ExplodeColumn(df, options).run()
...
def test_dataframe_with_missing_required_column():
    ...
    with pytest.raises(ValueError) as e:
        run_component(input_df, {"column_to_explode": "not_a_col"})
...
def test_input_dataframe_with_multiple_rows():
    input_df = pd.DataFrame(
        {
    ...
    result_df = run_component(input_df, {"column_to_explode": "col"})
